...
zokou({
  ...
  try {
    ...
    if (videos && videos.length > 0) {
      ...
      const apiResponse = await fetch(`https://gifted-apis-third-30b2fdbb9819.herokuapp.com/api/download/ytmp4?url=${encodeURIComponent(videoUrl)}&apikey=${giftedapikey}`);
...
zokou({
  ...
  try {
    ...
    if (videos && videos.length > 0) {
      ...
      const apiResponse = await fetch(`https://gifted-apis-third-30b2fdbb9819.herokuapp.com/api/download/ytmp3?url=${encodeURIComponent(videoUrl)}&apikey=${giftedapikey}`;
      const apiResult = await apiResponse.json();
...
zokou({
  ...
  try {
    ...
    if (videos && videos.length > 0) {
      ...
      // Call the API endpoint with the video URL to fetch audio download URL
      const apiResponse = await fetch(`${BaseUrl}/api/download/ytmp3?url=${encodeURIComponent(videoUrl)}&apikey=${giftedtechk}`);
      const apiResult = await apiResponse.json();
...
zokou({
  ...
  try {
    ...
    if (videos && videos.length > 0) {
      ...
      // Call the API endpoint with the video URL to fetch the video download URL
      const apiResponse = await fetch(`${BaseUrl}/api/download/ytmp4?url=${encodeURIComponent(videoUrl)}&apikey=${giftedtechk}`);
      const apiResult = await apiResponse.json();
